              ...
              <ul class="nav nav-pills nav-fill p-1" role="tablist">
                <li class="nav-item">
                  <a class="nav-link mb-0 px-0 py-1 active d-flex align-items-center justify-content-center "
                    data-bs-toggle="modal" data-bs-target="#addModal" href="javascript:;" aria-selected="true">
            ...
            <table id="dataTable" class="table align-items-center mb-0">
              <thead>
                <tr>
                  ...
                  <th class="text text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center ">Aksi
                  </th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <p class="text-xs text-secondary mb-0">{{ date_format(date_create($item->dari), 'H:i') }} s/d {{
                      date_format(date_create($item->sampai), 'H:i') }}</p>
                  </td>
                  <td>
                    <h6 class="text-xs font-weight-bold mb-0">{{ $item->hari }}</h6>
                    <p class="text-xs text-secondary mb-0">{{ $item->hari }}</p>
                  </td>
                  ...
                  <td class="d-flex justify-content-center">
                          ...
                          <div class="modal-content">
                            ...
                            <form action="schedule/delete/{{ $item->id }}" method="POST">
                              ...
                              <div class="modal-footer">
                                ...
                                <button type="button" class="btn bg-gradient-secondary text-white ml-auto"
                    ...
                    <a class="btn btn-sm bg-gradient-info text-white px-3 mb-0" href="javascript:;"
                      ...
                      data-original-title="Edit user"><i class="fas fa-pencil-alt me-2" aria-hidden="true"></i>Edit</a>
                    <!-- Modal -->
                            ...
                            <form action="{{  route('update.schedule', $item->id)  }}" method="POST">
                              ...
                              <div class="form-group col-lg">
                                ...
                                <select name="materi_id" id="materi_id" class="form-control">
                                  ...
                                  @foreach($materi as $pj)
                                  <option @if($item->materi_id == $pj->id) selected @endif value="{{ $pj->id }}">{{
                                    $pj->nama_materi}}</option>
                                  @endforeach
                                </select>
                              </div>
                              <div class="row">
                                <div class="col-6">
                                  <div class="form-group">
                                    <label for="dari">Dari Jam</label>
                                    <input type="time" class="form-control" id="dari" name="dari"
                                      value="{{ $item->dari }}" placeholder="">
                                  </div>
                                  <div class="form-group">
                                    <label for="sampai">Sampai Jam</label>
                                    <input type="time" class="form-control" id="sampai" name="sampai"
                                      value="{{ $item->sampai }}" placeholder="">
                                  </div>
                                </div>
                                <div class="col-6">
                                  <div class="form-group">
                                    ...
                                    <input type="date" class="form-control" id="hari" name="hari"
                                      value="{{ $item->hari }}" placeholder="">
                                  </div>
                                  <div class="form-group">
                                    <label for="is_active">Status</label>
                                    <select name="is_active" id="is_active" class="form-control">
                                      <option value="1" {{ $item->is_active == 1 ? 'selected' : '' }}>Aktif</option>
                                      <option value="0" {{ $item->is_active == 0 ? 'selected' : '' }}>Tidak Aktif
                                      </option>
                                    </select>
                                  </div>
                                </div>
